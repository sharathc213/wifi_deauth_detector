...
SCRIPT_TO_EXECUTE_PLUS_ARGS='manage.py runserver'
...
if [ -d "$VIRTUAL_ENV_DIR" ]; then
	...
	PYTHON_TO_USE="$VIRTUAL_ENV_DIR/bin/python3"
...
if [ ! -e "$OUTPUT_PID_PATH/$OUTPUT_PID_FILE" ]; then
	...
	sudo "$PYTHON_TO_USE" ./$SCRIPT_TO_EXECUTE_PLUS_ARGS > /dev/null 2>&1 & echo $! > "$OUTPUT_PID_PATH/$OUTPUT_PID_FILE" -s "kali"
